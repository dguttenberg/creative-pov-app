...
export async function POST(request: Request) {
  try {
    ...
    if (!BRAIN_ENDPOINT) {
      console.error("BRAIN_ENDPOINT env var is not set");
      return NextResponse.json(
        { error: "Server misconfigured: BRAIN_ENDPOINT is not set" },
        { status: 500 }
      );
    }

    console.log("Calling Brain endpoint", {
    ...
    try {
      brainResponse = await fetch(BRAIN_ENDPOINT, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ request_text: requestText }),
      });
    } catch (fetchErr: any) {
      console.error("Brain fetch threw:", fetchErr);
      return NextResponse.json(
        { error: `Failed to reach Brain: ${fetchErr.message}` },
        { status: 500 }
      );
    }

    if (!brainResponse.ok) {
      const body = await brainResponse.text().catch(() => "<unreadable>");
      console.error("Brain returned non-OK:", {
      ...
      return NextResponse.json(
        {
          error: `Failed to interpret request (Brain ${brainResponse.status}: ${body.slice(0, 200)})`,
        },
        { status: 500 }
      );
    }
